...
function quake:new(config)
  ...
  conf.argname    = conf.argname   or "-t %s"    -- how to specify window name
  ...
  conf.sticky     = conf.sticky    or false
...
function quake:display()
  ...
  for c in awful.client.iterate(function (c)
      ...
      return c.instance == self.name or c.name == self.name
                               end)
  do
    i = i + 1
    if i == 1 then
  ...
  if not client then
    ...
    self.props.tag = self.screen.selected_tag
    awful.spawn(cmd, self.props)
    return
  end
...
function quake:toggle()
  ...
  if current_tag and self.last_tag ~= current_tag and self.visible and not self.sticky then
